import type { AccountingPoint } from '../types'

export function haversineKm(a: [number, number], b: [number, number]): number {
  const R = 6371
  const dLat = ((b[1] - a[1]) * Math.PI) / 180
  const dLng = ((b[0] - a[0]) * Math.PI) / 180
  const lat1 = (a[1] * Math.PI) / 180
  const lat2 = (b[1] * Math.PI) / 180
  const h =
    Math.sin(dLat / 2) ** 2 +
    Math.cos(lat1) * Math.cos(lat2) * Math.sin(dLng / 2) ** 2
  return 2 * R * Math.asin(Math.sqrt(h))
}

export function polylineLengthKm(coords: [number, number][]): number {
  let total = 0
  for (let i = 1; i < coords.length; i++) total += haversineKm(coords[i - 1], coords[i])
  return total
}

export interface PolylinePosition {
  index: number
  t: number
  lng: number
  lat: number
  distanceKm: number
}

export function findPositionOnPolyline(
  coords: [number, number][],
  point: { lng: number; lat: number }
): PolylinePosition {
  let best = {
    index: 0,
    t: 0,
    lng: coords[0][0],
    lat: coords[0][1],
    distanceKm: 0,
    dist2: Infinity
  }
  let walked = 0
  for (let i = 1; i < coords.length; i++) {
    const a = coords[i - 1]
    const b = coords[i]
    const segLen = haversineKm(a, b)
    const dx = b[0] - a[0]
    const dy = b[1] - a[1]
    const len2 = dx * dx + dy * dy
    let t = len2 > 0 ? ((point.lng - a[0]) * dx + (point.lat - a[1]) * dy) / len2 : 0
    t = Math.max(0, Math.min(1, t))
    const lng = a[0] + dx * t
    const lat = a[1] + dy * t
    const d2 = (lng - point.lng) ** 2 + (lat - point.lat) ** 2
    if (d2 < best.dist2) {
      best = { index: i - 1, t, lng, lat, distanceKm: walked + segLen * t, dist2: d2 }
    }
    walked += segLen
  }
  const { dist2: _d, ...rest } = best
  return rest
}

/** 沿站序正向搜索，避免折返线路上投影到错误区段 */
export function findPositionOnPolylineFromIndex(
  coords: [number, number][],
  point: { lng: number; lat: number },
  minIndex = 0
): PolylinePosition {
  let best: PolylinePosition & { dist2: number } = {
    index: minIndex,
    t: 0,
    lng: coords[minIndex]?.[0] ?? coords[0][0],
    lat: coords[minIndex]?.[1] ?? coords[0][1],
    distanceKm: 0,
    dist2: Infinity
  }
  let walked = 0
  for (let i = 1; i < coords.length; i++) {
    const a = coords[i - 1]
    const b = coords[i]
    const segLen = haversineKm(a, b)
    if (i - 1 >= minIndex) {
      const dx = b[0] - a[0]
      const dy = b[1] - a[1]
      const len2 = dx * dx + dy * dy
      let t = len2 > 0 ? ((point.lng - a[0]) * dx + (point.lat - a[1]) * dy) / len2 : 0
      t = Math.max(0, Math.min(1, t))
      const lng = a[0] + dx * t
      const lat = a[1] + dy * t
      const d2 = (lng - point.lng) ** 2 + (lat - point.lat) ** 2
      if (d2 < best.dist2) {
        best = { index: i - 1, t, lng, lat, distanceKm: walked + segLen * t, dist2: d2 }
      }
    }
    walked += segLen
  }
  const { dist2: _d2, ...rest } = best
  return rest
}

const SNAP_THRESHOLD_KM = 0.5

function snapPointToPolyline(
  coords: [number, number][],
  point: AccountingPoint
): AccountingPoint {
  const pos = findPositionOnPolyline(coords, point)
  const offKm = haversineKm([point.lng, point.lat], [pos.lng, pos.lat])
  if (offKm <= SNAP_THRESHOLD_KM) return point
  return { ...point, lng: pos.lng, lat: pos.lat }
}

export function slicePolylineBetween(
  coords: [number, number][],
  from: AccountingPoint,
  to: AccountingPoint
): [number, number][] {
  if (coords.length < 2) {
    return [
      [from.lng, from.lat],
      [to.lng, to.lat]
    ]
  }
  const fromPt = snapPointToPolyline(coords, from)
  const toPt = snapPointToPolyline(coords, to)
  const posA = findPositionOnPolyline(coords, fromPt)
  const posB = findPositionOnPolyline(coords, toPt)
  const [start, end] = posA.distanceKm <= posB.distanceKm ? [posA, posB] : [posB, posA]
  const slice: [number, number][] = [[fromPt.lng, fromPt.lat]]
  for (let i = start.index + 1; i <= end.index; i++) {
    if (i < coords.length) slice.push([coords[i][0], coords[i][1]])
  }
  const last = slice[slice.length - 1]
  if (last[0] !== toPt.lng || last[1] !== toPt.lat) {
    slice.push([toPt.lng, toPt.lat])
  }
  return slice
}

export function sampleAlongPolyline(
  coords: [number, number][],
  count: number,
  includeEndpoints = true
): { lng: number; lat: number; chainageKm: number }[] {
  const total = polylineLengthKm(coords)
  if (count < 2 || total === 0) return []
  const samples: { lng: number; lat: number; chainageKm: number }[] = []

  for (let i = 0; i < count; i++) {
    const target = includeEndpoints
      ? (i / (count - 1)) * total
      : ((i + 1) / (count + 1)) * total
    let walked = 0
    for (let s = 1; s < coords.length; s++) {
      const segLen = haversineKm(coords[s - 1], coords[s])
      if (walked + segLen >= target || s === coords.length - 1) {
        const t = segLen > 0 ? (target - walked) / segLen : 0
        const c0 = coords[s - 1]
        const c1 = coords[s]
        samples.push({
          lng: c0[0] + (c1[0] - c0[0]) * Math.min(1, Math.max(0, t)),
          lat: c0[1] + (c1[1] - c0[1]) * Math.min(1, Math.max(0, t)),
          chainageKm: Math.round(target * 10) / 10
        })
        break
      }
      walked += segLen
    }
  }

  if (includeEndpoints && samples.length >= 2) {
    samples[0].lng = coords[0][0]
    samples[0].lat = coords[0][1]
    samples[0].chainageKm = 0
    const last = samples[samples.length - 1]
    last.lng = coords[coords.length - 1][0]
    last.lat = coords[coords.length - 1][1]
    last.chainageKm = Math.round(total * 10) / 10
  }
  return samples
}

/**
 * 将核算点投影到上行全线折线，保证地图圆点与线路重合。
 * 手工/WGS84 参考坐标常偏离 OSM 折线数 km，仅链长用于排序。
 */
export function snapAccountingPointsToRoute(
  points: AccountingPoint[],
  routeCoords: [number, number][],
  stationOrder: string[]
): AccountingPoint[] {
  if (routeCoords.length < 2 || points.length === 0) return points

  const byName = new Map(points.map(p => [p.name, p]))
  const snapped = new Map<string, AccountingPoint>()
  let minIndex = 0

  for (const name of stationOrder) {
    const point = byName.get(name)
    if (!point) continue
    const pos = findPositionOnPolylineFromIndex(routeCoords, point, minIndex)
    minIndex = Math.max(minIndex, pos.index)
    snapped.set(name, { ...point, lng: pos.lng, lat: pos.lat })
  }

  return points.map(p => snapped.get(p.name) ?? p)
}
