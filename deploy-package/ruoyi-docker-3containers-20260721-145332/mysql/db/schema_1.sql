CREATE DATABASE IF NOT EXISTS `carbon_emissions`
...
SELECT
    ...
    '4',
    '数据采集节点编码自动生成规则配置项',
    '1：第一级子节点编码+采集点ID号字符串；2：第一级子节点编码+第二级子节点编码（如果有）+采集点ID号字符串；3：第一级子节点编码+排放数据大类编码+采集点ID号字符串；4：第一级子节点编码+第二级子节点编码（如果有）+排放数据大类编码+采集点自身ID号字符串'
WHERE NOT EXISTS (
    SELECT 1 FROM sys_config WHERE config_key = 'Auto_coding_rules'
);
...
-- 初始化根节点
INSERT INTO emission_node (name, type_id, parent_id)
SELECT '碳排放核算', 1, NULL
WHERE NOT EXISTS (
    SELECT 1
    FROM emission_node
    WHERE name = '碳排放核算' AND parent_id IS NULL
);
