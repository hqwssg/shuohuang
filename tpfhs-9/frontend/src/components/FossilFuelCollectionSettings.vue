<template>
  <div class="fossil-fuel-collection-settings">
      ...
      <div v-if="selectedNode" class="detail-content">
        ...
        <el-descriptions :column="2" border>
          ...
          <template v-else-if="selectedNode.nodeType === 'meter'">
            <el-descriptions-item label="名称">{{ selectedNode.name }}</el-descriptions-item>
            <el-descriptions-item label="拼音编码">{{ selectedNode.pinyinCode || '-' }}</el-descriptions-item>
            <el-descriptions-item label="燃料类型">{{ selectedNode.fuelType || '-' }}</el-descriptions-item>
            ...
            <el-descriptions-item label="计费周期">
              {{ selectedNode.billingCycleLength || '-' }} {{ { 1: '周', 2: '月', 3: '季度', 4: '年' }[selectedNode.billingCycleUnit] || '' }}
              {{ selectedNode.billingCycleStartDate != null ? '（起始偏移：' + selectedNode.billingCycleStartDate + '）' : '' }}
            </el-descriptions-item>
            <el-descriptions-item label="状态">
              <el-tag :type="selectedNode.status === 1 ? 'success' : 'info'">
                {{ selectedNode.status === 1 ? '启用' : '停用' }}
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12">
              <el-form-item label="燃料类型">
                <el-select v-model="formData.fuelType" placeholder="请选择燃料类型" filterable style="width: 100%">
                  <el-option
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12">
              ...
              <el-form-item label="能耗用途">
                <el-select v-model="formData.energyUse" placeholder="请选择能耗用途" style="width: 100%">
                  <el-option label="生产用能" value="生产用能" />
                  <el-option label="生产辅助用能" value="生产辅助用能" />
            ...
            <el-col :span="8">
              <el-form-item label="计量单位">
                <el-select v-model="formData.measurementUnit" placeholder="请选择计量单位" filterable>
                  <el-option v-for="u in fossilFuelUnits" :key="u.unitCode" :label="u.unitName" :value="u.unitCode" />
                </el-select>
              </el-form-item>
            </el-col>
          </el-row>
...
const loadFuelTypeDict = async () => {
  try {
    const response = await axios.get('/api/data-dict/items/emission_subcategory')
    const items = response.data || []
    fuelTypeOptions.value = items
      .filter(item => item.parentCode === 'FF')
      .map(item => ({ code: item.code, value: item.value }))
  } catch (error) {
    console.error('加载燃料类型字典失败:', error)
    fuelTypeOptions.value = [
      { code: 'FF_B', value: '烟煤' },
      { code: 'FF_LB', value: '褐煤' },
      { code: 'FF_CK', value: '焦炭' },
      { code: 'FF_PC', value: '石油焦' },
      { code: 'FF_NG', value: '天然气' },
      { code: 'FF_LNG', value: '液化天然气' },
      { code: 'FF_LPG', value: '液化石油气' },
      { code: 'FF_G', value: '汽油' },
      { code: 'FF_D', value: '柴油' },
      { code: 'FF_C', value: '原油' },
      { code: 'FF_F', value: '燃料油' },
      { code: 'FF_BFG', value: '高炉煤气' },
      { code: 'FF_COG', value: '转炉煤气' },
      { code: 'FF_COK', value: '焦炉煤气' }
    ]
  }
}
...
const cascadeDialogVisible = ref(false)   // 级联关系弹窗显示状态
const cascadeForm = reactive({
  parentMeterId: 0                         // 当前选中的上级采集点ID，0表示无上级
})
const cascadeMeterList = ref([])           // 可选的上级采集点列表（同细分范围下，排除自身）

/**
 * 打开"更改采集点级联关系"弹窗
 * 从树中获取当前选中采集点的完整信息，加载同细分范围下的其他采集点作为可选上级
 */
...
/**
 * 保存采集点级联关系
 * 调用 PUT /meters/{id}/cascade 接口更新上级采集点，成功后关闭弹窗并刷新树
 */
...
const subScopeDialogVisible = ref(false)    // 细分范围变更弹窗显示状态
const subScopeForm = reactive({
  subScopeId: 0,                             // 当前选中的细分范围ID
  parentMeterId: 0                           // 当前选中的上级采集点ID，0表示无上级
})
const allSubScopesList = ref([])            // 当前采集范围下可选的细分范围列表
const subScopeMeterList = ref([])           // 选中细分范围下可选的采集点列表

/**
 * 打开"更改采集点所属细分范围"弹窗
 * 只加载当前采集点所属采集范围（collectionScopeId）下的细分范围，而非全部细分范围
 * 同时加载当前细分范围下的采集点作为可选上级
 */
...
/**
 * 加载指定细分范围下的采集点列表（排除当前操作的采集点）
 * @param excludeMeterId 需排除的采集点ID，通常为当前选中的采集点
 */
...
/**
 * 处理细分范围切换事件
 * 切换到新细分范围后，重置上级采集点为"无上级"，并重新加载该细分范围下的可选采集点
 * @param newSubScopeId 新选中的细分范围ID
 */
...
/**
 * 保存采集点所属细分范围变更
 * 调用 PUT /meters/{id}/sub-scope 接口更新细分范围和上级采集点
 * 后端会递归更新所有子孙采集点的所属细分范围
 */
...
/**
 * 在树形数据中递归查找指定ID的节点
 * @param nodes 树节点数组（通常为 treeData.value）
 * @param id    目标节点ID
 * @return 找到的节点对象，未找到时返回 null
 */
...
onMounted(async () => {
  ...
  await Promise.all([
    ...
    (async () => {
      try {
        // 化石燃料涵盖液体燃料、气体燃料、固体燃料三类，合并去重
        const [liquid, gas, solid] = await Promise.all([
          unitApi.listByCategory('LIQUID_FUEL'),
          unitApi.listByCategory('GAS_FUEL'),
          unitApi.listByCategory('SOLID_FUEL')
        ])
        const map = new Map()
        for (const u of [...(liquid.data || []), ...(gas.data || []), ...(solid.data || [])]) {
          if (!map.has(u.unitCode)) map.set(u.unitCode, u)
        }
        fossilFuelUnits.value = Array.from(map.values())
      } catch (err) {
        console.error('加载化石燃料标准单位失败', err)
      }
    })()
  ])
})
...
<style scoped>
.fossil-fuel-collection-settings {
  display: flex;
  width: 100%;
  height: 100%;
