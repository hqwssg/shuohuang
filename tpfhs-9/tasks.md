...
## Task 1: sys_emission_calc_node_summary 表 DDL + 增量脚本
...
**工作内容**：
1. 在 schema.sql sys_emission_collection_node_data 表定义之后（900 行附近 calc_unit_default 之前或之后，保持顺序合理），新增 sys_emission_calc_node_summary 的 CREATE TABLE：
   - id BIGINT PK AUTO_INCREMENT
   - calculation_template_id BIGINT NOT NULL FK→sys_emission_calculation_template(id)
   - calculation_node_id BIGINT NOT NULL FK→sys_emission_calculation_node(id)
   - source_node_id BIGINT（冗余 sys_emission_node 源 ID，便于按组织查询）
   ...
   - calculation_unit_code VARCHAR(30)（来自 sys_emission_calc_unit_default.calculation_unit）
...
**Test Requirements**：
- TR 1.1 (rule)：在一个空库执行 schema.sql 后，`SHOW CREATE TABLE sys_emission_calc_node_summary` 输出能看到全部 5 个数值列（direct_energy_value/direct_carbon_emission/subtotal_energy_value/subtotal_carbon_emission/carbon_emission_factor）与 4 个索引+唯一键。
- TR 1.2 (rule)：在一个"已执行过旧版 schema.sql 的库"（假设已存在 sys_emission_calc_unit_default 等所有旧表）执行 schema-update-20260903.sql 全部语句，不报错且不重复插入（CREATE TABLE IF NOT EXISTS + INSERT IGNORE 语义正确）。
...
**工作内容**：
1. CalcNodeSummary 实体：所有字段用 @Column + @Table(name="sys_emission_calc_node_summary")，遵循项目现有实体风格（@Data + jakarta.persistence.* + @PrePersist onCreate 设置 created_at 和默认值）。
...
4. 单位换算辅助：
   - 定义 UnitConversionService.convert(...)：`BigDecimal convert(String subcategoryCode, String fromUnit, String toUnit, BigDecimal value)`。核心查 sys_emission_unit_conversion `WHERE subcategory_code=? AND from_unit_code=? AND to_unit_code=?`。找不到 → 记录 WARN 并返回原值（系数 1）。
   - 若项目已有 sys_emission_calc_unit_default Repository，复用 `findBySubcategoryCode(String)` 拿到 calculation_unit；否则新建。
5. 注：项目已有 sys_emission_unit_conversion DDL，若无对应实体则需补 UnitConversion 实体与 Repository（与 FactorUnit/GhgUnit 的新建风格一致）。
...
2. 新建 CalcNodeSummaryProcessor（@Service、@Profile("job")—— 因为仅在 Job 线程池内被调度调用），`process(Long calcTemplateId)` 实现：
   a. 查询本次 calculation_template_id 下所有 sys_emission_calculation_node 快照节点（含 type_id、parent_id、node_id 源、name）。
   ...
   e. 对每个节点 `processOneNode(calcTemplateId, node)`：
      ...
      - e2. 取本节点直属的 **采集节点（type=3）** 子节点集合 → 找出其 calc_node_id → 一次性查询其所有 sys_emission_collection_node_data 行。
      ...
      - e8. 反查 subcategory 对应 emission_category（查 sys_emission_data_dict_item parent_code）并填列。
      - e9. 批量 INSERT / saveAll 到 sys_emission_calc_node_summary。
